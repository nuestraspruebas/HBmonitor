...
def add_hb_peer(_peer_conf, _ctable_loc, _peer):
    ...
    print(_peer_conf['CALLSIGN'])
    print(_peer_conf['LOCATION'])
    print(_peer_conf['TX_FREQ'])
    print(_peer_conf['RX_FREQ'])
    print(_peer_conf['SOFTWARE_ID'])
    print(_peer_conf['PACKAGE_ID'])
    print('---------------------')
...
#                CTABLE['PEERS'][system][timeSlot]['TYPE'] = 'G'+chr(322)+'os'
    ...
    if opcode == OPCODE['CONFIG_SND']:
        ...
        if p[0] == 'GROUP VOICE' and p[2] != 'TX':
            if p[1] == 'END':
                ...
                if LASTHEARD_INC:
                   if (p[5] != '260221' and p[5] != '260222' and p[5] != '260223' and p[5] != '260224' and p[5] != '260225' and p[5] != '260226' and p[5] != '260227' and p[5] != '260228' and p[5] != '260229') and  p[8] != '3' and int(float(p[9]))> 2: 
                      tgr=p[8]
                      master=p[3]
                      if (tgr == "9" and master == "XLX132-D"):
                            tg="5"
                            tgname=master
                      elif (tgr == "9" and master == "XLX260-A"):
                            tg="6"
                            tgname=master
                      else:
                            tg=tgr
                            tgname=alias_tgid(int(p[8]),talkgroup_ids)
                      log_lh_message = '{},{},{},{},{},{},{},TS{},TG{},{},{},{}'.format(_now, p[9], p[0], p[1], p[3], p[5], alias_call(int(p[5]), subscriber_ids), p[7], tg, tgname,p[6], alias_short(int(p[6]), subscriber_ids))
                      curDir = os.getcwd()
                      ...
                      f.write("<br><fieldset style=\"border-radius:8px; box-shadow:0 0 10px #999;background-color:#e0e0e0e0; text-algin: lef; margin-left:15px;margin-right:15px;font-size:14px;border-top-left-radius: 10px; border-top-right-radius: 10px;border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;\">\n")
                      f.write("<legend><b><font color=\"#000\">&nbsp;.: Ostatnio aktywne stacje :.&nbsp;</font></b></legend>\n")
                      f.write("<table style=\"width:100%; font: 10pt arial, sans-serif\">\n")
                      f.write("<TR style=\" height: 32px;font: 10pt arial, sans-serif; background-color:#9dc209; color:black\"><TH>Data</TH><TH>Godzina</TH><TH>TSlot</TH><TH>TG#</TH><TH>Nazwa TG</TH><TH>Znak (DMR-Id)</TH><TH>Imi&#x0119;</TH><TH>Czas TX (s)</TH><TH>&#x0179;r&#x0F3;d&#x0142;o ID</TH><TH>System</TH></TR>\n")
