...
echo "<TR><TH>&nbsp;&nbsp;Date<TH>Time<TH>Callsign (DMR-Id)<TH>Name<TH>TG#<TH>TG Name<TH>Dur TX (s)<TH>Slot<TH>Source<TH>System</TR>\n\n";
...
// output table
echo "<TR>".$s.'&nbsp;'.$date_eu[2].".".$date_eu[1].".".$date_eu[0].$s.'&nbsp;'.substr($log_time[$i],11,5).$s.'<font color=brown><b>'.$user_call[$i]."</b></font><font size=\"-1\"> (".$user_id[$i].")</font>".$s.TRIM($user_name[$i]).$s.'<font color=blue><b>'.$tg[$i].'</b></font>'.$s.'<font color=green><b>'.$tgname[$i].'</b></font>'.$s."<center>".round($transmit_timer[$i])."</center>".$s.$ts[$i].$s.$src_name[$i].$s.$system[$i]."</TR>\n";
}
